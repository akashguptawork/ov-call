...
export class RoomConfigComponent implements OnInit, OnDestroy {
	...
	initNicknameAndSubscribeToChanges() {
		this.log.w(document.referrer);	
		this.log.w(window.location.href);
		var splitted;
		...
		if (window.location.href.indexOf('-') > -1) {
			if(document.referrer.indexOf('lightning') > -1) {
				this.log.w(splitted[1]);	
				this.nicknameFormControl.setValue(splitted[1]);
				this.oVSessionService.updateUsersNickname(splitted[1]);
				return;
			} else if(document.referrer.indexOf('hc-lsg') > -1) {
				this.log.w(splitted[2]);	
				this.nicknameFormControl.setValue(splitted[2]);
				this.oVSessionService.updateUsersNickname(splitted[2]);
				return;
			} else {
