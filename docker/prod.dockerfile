...
RUN wget "https://github.com/akashguptawork/ov-call/archive/${BRANCH_NAME}.zip" -O ov-call.zip && \
    unzip ov-call.zip && \
    rm ov-call.zip && \
    mv ov-call-${BRANCH_NAME}/openvidu-call-front/ . && \
    mv ov-call-${BRANCH_NAME}/openvidu-call-back/ . && \
    rm openvidu-call-front/package-lock.json && \
    rm openvidu-call-back/package-lock.json && \
    rm -rf ov-call-${BRANCH_NAME} && \
